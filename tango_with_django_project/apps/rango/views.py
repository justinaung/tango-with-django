...
from apps.rango.forms import CategoryForm, PageForm, UserForm, UserProfileForm
...
def show_category(request, category_name_slug):
    ...
    # Go render the response and return it to the clietn.
...
def register(request):
    ...
    # True when registration succeds.
    registered = False

    # If it's a HTTP POST, we're insterested in processing form data.
    if request.method == 'POST':
...
def user_logout(request):
    # Since we know the user is logged in, we can now jsut log them out.
    logout(request)
